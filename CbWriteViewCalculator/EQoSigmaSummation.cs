...
{
	public class EQoSigmaSummation : oEquationObject
	{
		private int zzzSigmaWidth = 40;
		private int zzzSigmaHeight = 60;
		...
		public override Bitmap GetImage(oEquationDrawContext TheDrawContext)
		{
			...
			g.DrawLine(sigmaPen, sigmaLeft + 2, sigmaTop + this.zzzSigmaHeight - 4, sigmaLeft + this.zzzSigmaWidth - 2, sigmaTop + this.zzzSigmaHeight - 4); //dessous
			g.DrawLine(sigmaPen, sigmaLeft + 2, sigmaTop + 4, sigmaLeft + (this.zzzSigmaWidth / 2), sigmaTop + (this.zzzSigmaHeight / 2)); //diago up
			g.DrawLine(sigmaPen, sigmaLeft + 2, sigmaTop + this.zzzSigmaHeight - 4, sigmaLeft + (this.zzzSigmaWidth / 2), sigmaTop + (this.zzzSigmaHeight / 2)); //diago down
		...
		//void new()
		public EQoSigmaSummation()
		{
			this.zzzActualEquationObjectType = EquationObjectType.Function;
			this.zzzActualSpecificObjectType = SpecificObjectType.fSigmaSummation;
		...
		public EQoSigmaSummation(oEquation MainEq, oEquation UpEq, oEquation DownEq)
		{
			this.zzzActualEquationObjectType = EquationObjectType.Function;
			this.zzzActualSpecificObjectType = SpecificObjectType.fSigmaSummation;
		...
		{
			EQoSigmaSummation copy = new EQoSigmaSummation(this.equMain.GetCopy(), this.equUp.GetCopy(), this.equDown.GetCopy());
		...
		public override string GetSaveString()
		{
			...
			return "\\8sigmasum" + this.VarName + ";" + strqUp + strqDown + strqMain;
		}
		...
		public override void CompileToMSIL(CompileContext cc)
		{
			...
			//on met la variable de la sum à 0
			...
			//on importe au stack la somme actuel, on l'addition avec le terme actuel puis on renvoit ca dans la variable de la somme
